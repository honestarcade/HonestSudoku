...
List<String> engineImportOffenders(String path, String source) {
  ...
  for (final uri in directiveUris(source)) {
    if (uri.startsWith('dart:')) {
      if (!engineAllowedLibraries.contains(uri)) offenders.add('$path: $uri');
    } else if (uri.startsWith('package:')) {
      if (!uri.startsWith(engineAllowedPackage)) offenders.add('$path: $uri');
    } else if (uri.contains(':')) {
      offenders.add('$path: $uri');
    } else if (!_staysInEngine(path, uri)) {
      offenders.add('$path: $uri (leaves lib/engine/)');
    }
  }
  return offenders;
}

bool _staysInEngine(String path, String uri) {
  ...
  return parts.length >= 3 && parts[0] == 'lib' && parts[1] == 'engine';
}
