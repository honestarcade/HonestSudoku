...
format check, `flutter test --exclude-tags weekly,bench` (which includes the
invariant guards below; the `weekly` engine tier runs in its own scheduled
workflow), the release bundle build, and `tools/check_aab.sh` over that bundle. It must print
...
2. **Boards are generated on the device at runtime, never bundled, and every board has exactly one solution.** *(test-enforced: the engine property guards (`test/guards/engine_guard_*_test.dart`) run the real generator over a sample of seeds for every supported size and difficulty on every pull request, `prSeeds` in `engine_guard_support.dart` setting the sample, and have the solver count every board's solutions; `.github/workflows/engine-nightly.yml` runs the wider `weekly` tier — guard: #26 (merged))*
...
4. **Deterministic generation.** The same seed, size, and difficulty always produce the same board. *(test-enforced: `test/fixtures/golden_boards.json` pins ungraded carves and one graded board per supported size and difficulty, regenerated by the engine guards on every pull request; each property guard also regenerates seed 1 in a fresh generator; the engine may use no unseeded `Random` and no clock outside the off-thread wrapper — guard: #26 (merged))*
