...
- **Decision (#238, and it grew).** The two `#217` markers were vacuous because `'overflow'` is part of that test's name. Rather than fix two strings, the battery now audits every marker before it mutates anything: one suite run through the json reporter, which must be green, and no selected marker may be part of any test's name. It found **eighteen** distinct vacuous markers across about thirty-five entries — `'destroys'`, `'ruleset'`, `'refusal'`, `'command line'`, and four that were a test's name verbatim — all of which scored `caught` for any red suite whatever. Each was replaced with a prefix of the reason the assertion actually prints, taken from that mutation's own failure output rather than from reading.
  **And a correction to my own plan for it, made while building it:** the first version audited the markers against the *printed output* of a green run. The compact reporter truncates every line to a terminal width and CI's reporter does not, so that version would have refused in CI and passed locally — the machine-dependence #217 was reverted for once already. It audits the NAMES, which come from the source.
- **Decision (#237, correcting the issue I filed).** A control cannot be protected by mutating the control: that makes the suite greener, so the entry SURVIVES by construction. What protects one is a defect nothing else catches — `refuse` printing its refusal and then exiting 0, `secrets_present` that can never succeed. Two entries. The sweep found three of the five controls already had such an entry, and one — the leak test's — **cannot** have one, because every natural defect that stops a secret-bearing step from completing also fails the propagation vacuity check. Recorded rather than faked with a contrived entry.
...
- **Decision (#241 item 3, which I had left as the owner's call and am taking):** `HS_RULESET_READ_TOKEN` stays on the `mutations` job. Without it that job's ruleset test skips the bypass block, so every mutation reaching it would be scored against a test that stopped early. Read-only, one repository, one permission.
...
- **The digest shape stays open, recorded where it bites:** hashing a secret into a summary is a real leak for a human-chosen password, and catching it needs a sha256 implementation, which is a dependency to justify against invariant 3.
